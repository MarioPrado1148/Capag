...
#st.title("<h1 style='text-align: center; color: black;'>'Classificação da capacidade de pagamento dos municípios brasileiros com base em dados geoeconômicos'</h1>", unsafe_allow_html=True)
...
if radio == 'Visão cientista de dados':
	...
	if select_event_cientista == 'Coleta de Dados':
		...
		select_radio_cientista = st.sidebar.radio('Selecione a variável',['Região','Região Metropolitana', 'Mun_Reg_Geog_Imediata','Região Rural'])
		...
		elif select_radio_cientista == 'Região Rural':
			col1, col2 = st.columns(2)

			with col1:
			    st.markdown("<h1 style='text-align: center; color: blue;'>Variável 'Região Rural'</h1>", unsafe_allow_html=True)
			    sns.countplot(x = "Região_rural", data = df_streamlit).set_xticklabels(chart.get_xticklabels(), rotation=45)
			    st.pyplot()
